...
# ------------------------------------------------------------------------------
    def construct(self, d_init_struct):
        ...
        self.i_is_outpost               = d_init_struct['is_outpost']
        ...
        self.___i_max_population        = d_init_struct['__max_population']
...
# ------------------------------------------------------------------------------
    def init_available_production(self, game_rules, players):
        ...
        known_techs     = players[self.owner_id].known_techs
        replaced        = []
...
# ------------------------------------------------------------------------------
    def xget_colony_pollution(self, production, PLAYERS):
        ...
        if TECH_NANO_DISASSEMBLERS in PLAYERS[self.owner_id]['known_techs']:
            tolerance += tolerance
...
# ------------------------------------------------------------------------------
    def exists(self):
        return self.owner_id < 0xff
# ------------------------------------------------------------------------------
    def is_owned_by(self, player_id):
        return self.exists() and self.owner_id == player_id
# ------------------------------------------------------------------------------
    def is_outpost(self):
        return self.exists() and self.is_outpost
# ------------------------------------------------------------------------------
    def is_colony(self):
...
# ------------------------------------------------------------------------------
    def update_industry_progress(self):
        ...
        self.industry_progress += self.industry
        return self.industry_progress
# ------------------------------------------------------------------------------
    def reset_industry_progress(self):
        self.industry_progress = 0
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
    def add_build_item(self, production_id, flags = 0):
        if len(self.build_queue) < 7:
           self.build_queue.append({'production_id': production_id, 'flags': flags})
# ------------------------------------------------------------------------------
    def remove_build_item(self, production_id):
        build_queue_ids = self.build_queue_ids
        if production_id in build_queue_ids:
            self.build_queue.pop(build_queue_ids.index(production_id))
# ------------------------------------------------------------------------------
    def get_build_item(self):
        if len(self.build_queue) < 1:
            return None
        elif self.build_queue[0]['production_id'] == 0xFF:
            return None
        elif self.build_queue[0]['production_id'] == 249:  # repeat
            return self.build_queue[1]
        else:
            return self.build_queue[0]
# ------------------------------------------------------------------------------
    def in_build_queue(self, production_id):
        return production_id in self.build_queue_ids
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
    def get_population_growth(self, max_populations, PLAYERS):
        ...
        if 192 in PLAYERS[self.owner_id].known_techs:
...
# print "raise_population ... pop_grow: %s" % str(self.pop_grow)
        for race in range(8):
            self.pop_raised[race] += self.pop_grow[race]
            if self.pop_raised[race] > 999:
...
# ------------------------------------------------------------------------------
    def recount(self, game_rules, colony_leader, players):
        ...
        """
        print "$$$ colony::recount() ... colony_id = %i" % self.obj_id
        ...
        if not self.exists():
            self.population      = 0
            self.max_populations = rules.get_empty_max_populations()
            return
        ...
        self.morale_summary   = rules.compose_morale_summary(game_rules, self, colony_leader, players)
        self.morale           = rules.count_summary_result(self.morale_summary)
        self.food_summary     = rules.compose_food_summary(game_rules, self, players)
        self.i_food           = rules.count_summary_result(self.food_summary)
        self.industry_summary = rules.compose_industry_summary(game_rules, self, colony_leader, players)
        self.i_industry       = rules.count_summary_result(self.industry_summary)
        self.pollution        = self.industry_summary['pollution']
        self.bc_summary       = rules.compose_bc_summary(game_rules, self, players)
        self.i_bc             = rules.count_summary_result(self.bc_summary)
        self.research_summary = rules.compose_research_summary(game_rules, self, players)
        self.i_research       = rules.count_summary_result(self.research_summary)
        self.max_populations  = rules.compose_max_populations(self, players)
        self.max_population   = max(self.max_populations)
        self.pop_grow         = self.get_population_growth(self.max_populations, players)
